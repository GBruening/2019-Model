...
elseif N == 0
    ...
    if nargout == 0
%         disp([repmat(char(8), 1, (w+11)), char(10), '100%[', repmat('=', 1, w+1), ']']);
    end
else
    ...
    progress = fscanf(f, '%d');
    fclose(f);
    percent = ((length(progress)-1)/2)/progress(1)*100;
    
    f = fopen('parfor_progress.txt', 'a');
    fprintf(f, strcat(string(floor(percent/10)*10),' \n'));
    fclose(f);
    ...
    if nargout == 0
        f = fopen('parfor_progress.txt');
        prog_str = textscan(f,'%s');
        if length(find(ismember(prog_str{1,1},string(floor(percent/10)*10))))==1
...
%         disp([repmat(char(8), 1, (w+k)), char(10), perc, '[', repmat('=', 1, round(percent*w/100)), '>', repmat(' ', 1, w - round(percent*w/100)), ']']);
            perc = sprintf('%.0f%%', percent); % 4 characters wide, percentage
            disp(perc);
        end
        fclose(f);
    end

...
fold = pwd;
load('Resamp_data_apr2021_nobuff.mat');
...
if exist('rnjesus');
    if rnjesus
        filename = sprintf('aa_%scost_04-29-2021_rng',  minparams{L});
    else
        filename = sprintf('aa_%scost_04-29-2021_mj',  minparams{L});
    end
else
    rnjesus = 0;
    filename = sprintf('aa_%scost_04-29-2021_mj',  minparams{L});
end    
filename = sprintf('aa_%scost_04-29-2021_210normstress',  minparams{L});

fprintf('Processing %s \n',filename);
clearvars -except time_step target_str input_normforce masses minparams L Resamp tic fold minfail rnjesus filled emptys
...
[Data{c,subj,s,t}] = Gen_mvt_gb(Resamp,c,subj,s,t,ro,rf,vars{c,subj,s,t}.time_inc,vars{c,subj,s,t});

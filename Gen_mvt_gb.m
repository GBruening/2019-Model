function[Data]=Gen_mvt_gb(Resamp,c,subj,s,t, ro, rf, time_inc, vars)

re_size = length(squeeze(Resamp.P(c,subj,s,t,:)));
if isfield(Resamp,'X')
    x = reshape(Resamp.X(c,subj,s,t,:)-Resamp.X(c,subj,s,t,1),re_size,1)+vars.center(1);
    y = reshape(Resamp.Y(c,subj,s,t,:)-Resamp.Y(c,subj,s,t,1),re_size,1)+vars.center(2);

	buff = 0.05;

	x1 = spline([0,.0025,reshape(buff+Resamp.T(c,subj,s,t,6:end),1,length(Resamp.T(c,subj,s,t,6:end)))],...
	    [ro(1);ro(1);x(6:end)],[0:time_inc:Resamp.T(c,subj,s,t,end)])';
	y1 = spline([0,.0025,reshape(buff+Resamp.T(c,subj,s,t,6:end),1,length(Resamp.T(c,subj,s,t,6:end)))],...
	    [ro(2);ro(2);y(6:end)],[0:time_inc:Resamp.T(c,subj,s,t,end)])';
%     length(x1)

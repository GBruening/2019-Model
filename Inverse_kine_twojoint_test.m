...
for L = length(minparams)
tic
filename = sprintf('aa_%scost2_03-22-2018',  minparams{L});

fprintf('Processing %s \n',filename);
clearvars -except time_step target_str input_normforce masses minparams L Resamp tic
for c=1:4
    for s=1:6
        for t = 1:8
            for d = 2:2
...
if exist('D:\Users\Gary\Google Drive\Muscle modeling\Min_jerk files')==7
    cd('D:\Users\Gary\Google Drive\Muscle modeling\Min_jerk files');
else
    cd('C:\Users\Gary\Google Drive\Muscle modeling\Min_jerk files');
end
toc 
end
